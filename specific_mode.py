...
def course_watch(courses, opener):  # courses--list of course info
    global parser
    from main import watch_logger
    result = []

    for coursesCnt in range(len(courses)):
        post_params = {}
        if courses[coursesCnt]["courseType"] == 'freeCourse':
            post_params = {'searchtj': str(courses[coursesCnt]['kxh']), 'xq': '0', 'jc': '0', 'kclbdm': ''}
        elif courses[coursesCnt]["courseType"] == 'planCourse':
            post_params = {'kch': str(courses[coursesCnt]['kxh']), 'xq': '0', 'jc': '0', 'kclbdm': '',
                           'jhxn': str(courses[coursesCnt]['zxjxjhh'])}
