...
# 重构思路：1.按用户刷新课余量，可避免每次的Cookie验证开销（此处正选时，查询时如果Cookie有问题报什么，如果不报，说明查询没有做鉴权）
# 2.提交操作采用Celery异步方式，避免阻塞正常刷新（此处考虑清楚，当一个课程被提交到队列中后应该不再刷新它，避免race）
# 前两个已实现，现在：按用户刷新会造成同用户多课程之间的阻塞，考虑移除掉验证机制，直接将每门课提交到队列
# TODO：直接在watch的时候抓异常，判断（cookie失效、密码错误、HTTP错误等）并进行相应处理
